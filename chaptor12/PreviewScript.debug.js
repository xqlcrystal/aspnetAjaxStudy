...
function _loadTypeDescriptorCompatLayer(w) {
    ...
    Sys.Preview.TypeDescriptor.createParameter = function Sys$Preview$TypeDescriptor$createParameter(parameterName, parameterType, isDomElement) {
        return Sys.Preview.TypeDescriptor._createParameter(parameterName.toLowerCase(), parameterType, isDomElement);
    }
}
...
window.XMLDOM = function window$XMLDOM(markup) {
    if (window.DOMParser) {
        return new window.DOMParser().parseFromString(markup, 'text/xml')
    }
    else if (window.XMLDOMParser) {
        return new window.XMLDOMParser().parseFromString(markup, 'text/xml')
    }
    else {
...
Sys.Preview.TypeDescriptor.prototype.addEvent = function Sys$Preview$TypeDescriptor$addEvent(eventName) {
    ...
    this._getEvents()[eventName] = { name: eventName };
}
...
Sys.Preview.TypeDescriptor.prototype.addProperty = function Sys$Preview$TypeDescriptor$addProperty(propertyName, propertyType, readOnly, isDomElement, associatedAttributes) {
    ...
    this._getProperties()[propertyName] = { name: propertyName, type: propertyType, 'readOnly': readOnly, 'isDomElement': isDomElement, attributes: attribs };
}
...
Sys.Preview.TypeDescriptor.createParameter = function Sys$Preview$TypeDescriptor$createParameter(parameterName, parameterType, isDomElement) {
    ...
    if (parameterType === Sys.UI.DomElement) {
                throw Error.argumentType("parameterType", Sys.UI.DomElement, Object, "Use isDomElement with a null type for element parameters.\ne.g., for descriptors use { name: 'foo', isDomElement: true, type: null }");
    }
    return { name: parameterName, type: parameterType, 'isDomElement': isDomElement };
}
    ...
    if (descriptor.properties) {
        ...
        for (var i = 0; i < length; i++) {
            ...
            if (! td._getProperties()[propertyName]) {
                ...
                if(associatedAttributes) {
                    Array.addRange(args, associatedAttributes);
                }
...
Sys.Preview.TypeDescriptor.setProperty = function Sys$Preview$TypeDescriptor$setProperty(instance, propertyName, value, key) {
    ...
    if (!td) {
            ...
            if(key.indexOf('.') === -1) {
                instance[propertyName][key] = value;
            }
            else {
    ...
    if (key) {
        ...
        if(!setter) {
            throw Error.invalidOperation(String.format('Set accessor was not found for "{0}" property on object of type "{1}"', propertyName, Object.getTypeName(instance)));
        }

                value = Sys.Preview.TypeDescriptor._evaluateValue(propertyInfo.type, propertyInfo.isDomElement, value);
    ...
    var method = instance[methodInfo.name];
        ...
        for (var i = 0; i < methodInfo.parameters.length; i++) {
            ...
            var value = parameters[parameterInfo.name];
                        ...
                        value = Sys.Preview.TypeDescriptor._evaluateValue(parameterInfo.type, parameterInfo.isDomElement, value);
...
Sys.Preview.TypeDescriptor._evaluateValue = function Sys$Preview$TypeDescriptor$_evaluateValue(targetType, isDomElement, value) {
            if(!targetType) {
        return value;
    }
...
Sys.Preview.CollectionChangedEventArgs = function Sys$Preview$CollectionChangedEventArgs(action, changedItem) {
    /// <param name="action" type="Sys.Preview.CollectionChangedAction"></param>
    /// <param name="changedItem" type="Object" mayBeNull="true"></param>
    var e = Function._validateParams(arguments, [
        {name: "action", type: Sys.Preview.CollectionChangedAction},
    ...
    function Sys$Preview$CollectionChangedEventArgs$get_action() {
        /// <value type="Sys.Preview.CollectionChangedAction"></value>
        if (arguments.length !== 0) throw Error.parameterCount();
        return this._action;
    }
    ...
    function Sys$Preview$UI$Data$ListViewRenderTask$execute() {
        ...
        for (; this._currentIndex < lastElementToRender; this._currentIndex++) {
            ...
            if (this._itemTemplate) {
                ...
                $addHandler(element, "focus", this._itemFocusHandler);
                $addHandler(element, "click", this._itemClickHandler);
                                            }
        ...
        if (this._itemElements) {
            for (var i = this._itemElements.length - 1; i >= 0; i--) {
                if (this._itemFocusHandler) {
                    $removeHandler(this._itemElements[i], "focus", this._itemFocusHandler);
                }
                if (this._itemClickHandler) {
                    $removeHandler(this._itemElements[i], "click", this._itemClickHandler);
                }
            }
        }
